...
//Manages player movement controls, sets player movement speed, plays movement sound effects 
...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	private float speedAmtY = 1.0f*mousespeed;//current player speed per axis which is applied to rigidbody velocity vector
	private float speedAmtX = 1.0f*mousespeed;
	[HideInInspector]
	...
	void Update(){
    	...
    	if(Input.GetKeyDown (FPSPlayerComponent.crouch) && !swimming && !climbing){
    		if(!crouchState){
    			if(!crouched){
    				crouched = true;
					...
					if(!Physics.CapsuleCast (p1, p2, capsule.radius * 0.9f, myTransform.up, out hit, 0.4f + playerHeightMod, clipMask.value)){
    					crouched = false;
					}
    			}
    			crouchState = true;
    		...
    		crouchState = false;
    		if((sprintActive || climbing || swimming) && !Physics.CapsuleCast (p1, p2, capsule.radius * 0.9f, myTransform.up, out hit, 0.4f + playerHeightMod, clipMask.value)){
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if (grounded) {
				...
				&& !Physics.CapsuleCast (p1, p2, capsule.radius * 0.9f, myTransform.up, out hit, 0.4f + playerHeightMod, clipMask.value)){
	    			crouched = false;
				...
				if((sprintActive && Input.GetKey(FPSPlayerComponent.fire))//cancel sprint if fire button is pressed
				|| (sprintActive && Input.GetKey(FPSPlayerComponent.reload))//cancel sprint if reload button is pressed
				|| (sprintActive && Input.GetKey(FPSPlayerComponent.zoom) && WeaponBehaviorComponent.canZoom)//cancel sprint if zoom button is pressed
				...
				//determine if player can run (check button input or Unity's input axes based on useAxisInput var of FPSPlayer.cs)
				...
				&& grounded){
					...
					if(sprintStopState){
				 	...
				 	canRun = false;
					if(staminaForSprintAmt < staminaForSprint && limitedSprint){
			...
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				&& (!rayTooSteep || inWater)){//do not jump if ground normal is greater than slopeLimit and not in water
					...
					rigidbody.velocity = new Vector3(velocity.x, Mathf.Sqrt(2.0f * jumpSpeed * gravity), velocity.z);
				}
			...
			if((climbing || swimming) && !jumping){//climbing var is managed by a ladder script attatched to a trigger that is placed near a ladder
				if((!FPSPlayerComponent.useAxisInput && Input.GetKey(FPSPlayerComponent.moveForward)) || (FPSPlayerComponent.useAxisInput && inputY > 0)){//only climb if player is moving forward
					...
					rigidbody.velocity = new Vector3(velocity.x, verticalSpeedAmt, velocity.z);
			...
			if(holdingBreath){
				...
				if(Time.time - diveStartTime > holdBreathDuration){
					if(drownStartTime < Time.time){
						FPSPlayerComponent.ApplyDamage(7.0f); 
			...
			if((grounded || climbing || swimming || ((airTime + 0.3f) > Time.time)) && FPSPlayerComponent.hitPoints > 1.0f && !FPSPlayerComponent.restarting){
				...
				if(FPSPlayerComponent.hitPoints < 1.0f || FPSPlayerComponent.restarting){		
					rigidbody.freezeRotation = false;//allow player's rigidbody to be pushed by forces and explosions after death
				...
				if(!swimming){
					...
					if(swimStartTime + 0.2f > Time.time){//make player sink under surface for a short time if they jumped in deep water 
						...
						if(landStartTime + 0.3f > Time.time){//make sure that player doesn't try to sink into the ground if wading into water
							if(!Physics.CapsuleCast (p1, p2, capsule.radius * 0.9f, -myTransform.up, out capHit, capsuleCastHeight, clipMask.value)){
								rigidbody.AddForce(new Vector3 (0, -6.0f, 0), ForceMode.VelocityChange);//make player sink into water after jump
							}
						}
					}else{	
						...
						rigidbody.AddForce(new Vector3 (0, swimmingVerticalSpeed, 0), ForceMode.VelocityChange);
						
					}
					rigidbody.useGravity = false;//don't use gravity when swimming	
				}
			}else{
