...
public class WeaponBehavior : MonoBehaviour {
	...
	//true when weapon has been picked up from an item that is not destroyed on use to prevent dropping this weapon 
	...
	void Start (){
		...
		if(!unarmed){
			...
			if(meleeSwingDelay == 0){//initialize muzzle flash color if not a melee weapon
				...
				if(muzzleLightObj){
					muzzleLightObj.light.enabled = false;
				}
		...
		if(Time.timeScale > 0 && Time.deltaTime > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed){
				...
				if(FPSPlayerComponent.hitPoints >= 1.0f){
					...
					&& !Input.GetKeyDown (FPSPlayerComponent.fire)
					...
					|| (FPSWalkerComponent.climbing && FPSWalkerComponent.lowerGunForClimb)){//cancel reload if player is climbing and weapon is lowered
				    	if(IronsightsComponent.reloading){
							...
							if(bulletsToReload != bulletsPerClip){
								...
								weaponMesh.animation["Neutral"].speed = 1.5f;
								weaponMesh.animation.Play("Neutral", PlayMode.StopAll);//play Neutral animation	
							}	
					...
					if(!FPSWalkerComponent.climbing){//dont shoot if player is climbing
						...
						||Input.GetKey (FPSPlayerComponent.fire)){
							...
							//sprinting
							if((!FPSPlayerComponent.useAxisInput && (Input.GetKey (FPSPlayerComponent.moveForward) || Input.GetKey (FPSPlayerComponent.moveBack)))
					...
					if (Input.GetKey(FPSPlayerComponent.fireMode)){
						...
						&& canShoot
						&& !IronsightsComponent.reloading
						&& ((!PistolSprintAnim && animation["RifleSprinting"].normalizedTime < 0.35f)//only change fire mode when weapon is centered
						 ||(PistolSprintAnim && animation["PistolSprinting"].normalizedTime < 0.35f))){
				...
				if ((canShoot || (FPSWalkerComponent.climbing && FPSWalkerComponent.lowerGunForClimb))//allow gun to stay centered and lowered when climbing
				|| FPSWalkerComponent.crouched
				...
				|| FPSWalkerComponent.cancelSprint){
					if(sprintAnimState){//animate weapon up
						...
						if(!PistolSprintAnim){
							...
							animation.CrossFade("PistolSprinting", 0.35f,PlayMode.StopAll);	
						}
	...
	void Update (){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(!unarmed && FPSPlayerComponent.hitPoints > 0){
				...
				if(muzzleFlash){
					if (muzzleFlash.renderer.enabled){
						if(muzzleFlashColor.a > 0.0f){
							...
							muzzleFlash.renderer.enabled = false;//disable muzzle flash object after alpha has faded
						}	
					}
				}
				...
				if(muzzleLightObj){
					if(muzzleLightObj.light.enabled){
						if(muzzleLightObj.light.intensity > 0.0f){
							if(shootStartTime + muzzleLightDelay < Time.time){
								muzzleLightObj.light.intensity -= muzzleLightReduction * Time.deltaTime;	
							}
						}else{
							muzzleLightObj.light.enabled = false;
						}
				...
				if (Input.GetKey (FPSPlayerComponent.fire) && !waterFireState){
					if(semiAuto){
						if(!semiState){
							Fire();
							semiState = true;
	...
	void MuzzFlash (){
		...
		if (muzzleFlash){
			...
			if(!FPSWalkerComponent.holdingBreath){
				if(muzzleLightObj){
					muzzleLightObj.light.enabled = true;
					muzzleLightObj.light.intensity = 8.0f;
				}
				
				//emit smoke particle effect from muzzle
				if(muzzleSmokeParticles){
					muzzleSmokeColor.a = muzzleSmokeAlpha;
					muzzleSmokeParticles.renderer.material.SetColor("_TintColor", muzzleSmokeColor);
	...
	IEnumerator SpawnShell (){
		...
		if(shell.rigidbody){
			shell.rigidbody.AddForce((transform.TransformDirection(shellEjectDirection) * shellForce), ForceMode.Impulse);
		}
		//Initialize object references for instantiated shell object
		shell.GetComponent<ShellEjection>().playerObj = playerObj;
		shell.GetComponent<ShellEjection>().lerpShell = shell2.transform;//pass reference of mesh shell object to RB shell object's ShellEjection.cs component
	...
	void Fire (){
		...
		if(FPSWalkerComponent.holdingBreath && !fireableUnderwater){ 
			if(cantFireState){
				otherfx.volume = 1.0f;
				otherfx.pitch = 1.0f;
				otherfx.clip = noammoSnd;
	   	...
	   	if (canShoot){
			...
			if(meleeSwingDelay == 0){//if this is not a melee weapon
				...
				if(swingSide){//determine which side to swing melee weapon
					...
					weaponMesh.animation.Rewind("MeleeSwingRight");
					weaponMesh.animation["MeleeSwingRight"].speed = fireAnimSpeed;//set weapon swing animation speed
					weaponMesh.animation.CrossFade("MeleeSwingRight", 0.1f,PlayMode.StopAll);//play weapon swing animation
					...
					weaponMesh.animation.Rewind("MeleeSwingLeft");
					weaponMesh.animation["MeleeSwingLeft"].speed = fireAnimSpeed;//set weapon swing animation speed
					weaponMesh.animation.CrossFade("MeleeSwingLeft",0.1f,PlayMode.StopAll);//play weapon swing animation
	...
	IEnumerator Reload (){
		...
		if(Time.timeSinceLevelLoad > 2){//prevent any reloading behavior at level start 
			...
			|| FPSWalkerComponent.cancelSprint)
			&& !(FPSWalkerComponent.climbing && FPSWalkerComponent.lowerGunForClimb)){//dont allow reloading if gun is lowered while climbing
	...
	void WeaponKick (){
		...
		mainCamTransform.localRotation = Quaternion.Slerp(myTransform.localRotation, kickRotation, 0.1f);
	}
