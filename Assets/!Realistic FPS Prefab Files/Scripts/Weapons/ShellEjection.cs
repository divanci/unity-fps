...
public class ShellEjection : MonoBehaviour
{
	...
	void Update (){
		...
		tempPos = Vector3.Lerp(tempPos, myTransform.position, Time.smoothDeltaTime * 64.0f);
		lerpShell.position = tempPos;
		tempRot.x = Mathf.LerpAngle(tempRot.x, myTransform.eulerAngles.x, Time.smoothDeltaTime * 64.0f);
		tempRot.y = Mathf.LerpAngle(tempRot.y, myTransform.eulerAngles.y, Time.smoothDeltaTime * 64.0f);
		tempRot.z = Mathf.LerpAngle(tempRot.z, myTransform.eulerAngles.z, Time.smoothDeltaTime * 64.0f);
	...
	void OnCollisionEnter(Collision collision){
		...
		if(collision.gameObject.layer == 15 || playerObjTransform.parent != null){
			Object.Destroy(lerpShell.gameObject);
			Object.Destroy(gameObject);
		}
	}
